...
def runExperiments(configFile, experimentsFolder, outputName):
    ...
    mutationProbabilities = list(np.arange(data['min_MutationProbability'], data['max_MutationProbability'] + 0.01,
                                           data['step_MutationProbability']))
    swapProbabilities = list(
        np.arange(data['min_SwapProbability'], data['max_SwapProbability'] + 0.01, data['step_SwapProbability']))
    ...
    headings = ['numberOfStudents', 'numberOfSupervisors', 'quotaSum', 'testCase', 'popSize', 'mutProb', 'swapProb',
                'crossoverOp', 'kValue', 'initial_maxFst', 'initial_minFst',
                'initial_maxFsup', 'initial_minFsup', 'final_maxFst', 'final_minFst', 'final_maxFsup', 'final_minFsup',
                'diversity', 'numberOfGenerations', 'total_time_generations',
                'avg_time_generation',
    ...
    for filename in files:
        ...
        identifier = str(expDetails['numberOfStudents']) + "\t" + str(expDetails['numberOfSupervisors']) + "\t" + str(
            expDetails['quotaSum']) + "\t" + str(expDetails['testCase']) + "\t"
                ...
                for swapProb in swapProbabilities:
                    ...
                    for crossoverOp in crossoverOperators:
                        ...
                        if crossoverOp == 'crossover' or crossoverOp == 'uniform' or crossoverOp == 'sp_crossover' or crossoverOp == 'none':
                            ...
                            testname = str(popSize) + "\t" + str(mutProb) + "\t" + str(
                                swapProb) + "\t" + crossoverOp + "\t" + str(-1)

                            crOp = strToOp(crossoverOp, 'crossover') if crossoverOp != 'none' else None
                            crossover_flag = False if crossoverOp == 'none' else True

                            geneticAlgorithm = GeneticAlgorithm(students, supervisors, fitnessCache, rankWeights, muOp,
                                                                crOp, selOp, crossover=crossover_flag)

                            (metricData, solutions) = geneticAlgorithm.start(popSize, genLimit, mutProb, swapProb,
                                                                             transferProb, population)
                            ...
                            for kVal in k_values:
                                testname = str(popSize) + "\t" + str(mutProb) + "\t" + str(
                                    swapProb) + "\t" + crossoverOp + "\t" + str(kVal)

                                geneticAlgorithm = GeneticAlgorithm(students, supervisors, fitnessCache, rankWeights,
                                                                    muOp, crOp, selOp)

                                (metricData, solutions) = geneticAlgorithm.start(popSize, genLimit, mutProb, swapProb,
                                                                                 transferProb, population, kVal)
