...
def createGAMSFileSup(filename, output, maxVersion=True):
    ...
    f.write(
        "quality ..\t z =e= (1/ (1+sqrt((1/n_supervisors) * sum(j, power(avg_wl-wl(j),2) )))**2 ) *  (1/n_supervisors) * sum(j,q(j)) ;\n")
...
def runExperimentsOpt(filename, studentVersion=True, maxVersion=True):
    ...
    model.capacity = Param(model.supervisors, initialize={s: supervisors[s].getQuota() for s in supervisors},
                           doc="Supervisor quotas")
    ...
    model.valuations = Param(model.students, model.supervisors, initialize=valuations,
                             doc="Values given to being assigned")

    model.n_students = Param(initialize=n_stu)
    model.n_supervisors = Param(initialize=n_sup)

    # Defining variables
    model.x = Var(model.students, model.supervisors, domain=BooleanVar, doc="Variables")
    ...
    if studentVersion:
        ...
        def objective_rule(model):
            ...
            for supervisor in model.supervisors:
                s = sum([1 / model.n_supervisors * model.valuations[i, supervisor] * model.x[i, supervisor] for i in
                         model.students]) / sum(model.x[i, supervisor] for i in model.students)
                if e is None:
                    e = s
                else:
                    e = e + s
            ...
            penalization = None
            for supervisor in model.supervisors:
                p = (sum(model.x[i, supervisor] for i in model.students) / model.capacity[supervisor] - avg_wl) * (
                            sum(model.x[i, supervisor] for i in model.students) / model.capacity[supervisor] - avg_wl)
...
def runAllExperimentsOptStudent(foldername):
    ...
    for name in filenames:
        ...
        print(str(expDetails["numberOfStudents"]) + "\t" + str(expDetails["numberOfSupervisors"]) + "\t" + str(
            expDetails["quotaSum"]) +
