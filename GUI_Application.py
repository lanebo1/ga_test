...
class Window(Frame):
    ...
    def runGA(self):
        ...
        if self.stuFile != None or self.supFile != None or self.outputName != None or self.keywordsFile != None:
            ...
            showinfo("Success",
                     "Genetic Algoritm Evolution completed. The results have been saved in " + filename + ".")
    ...
    def updateConfig(self):
        updateConfigFile("configGA.json", int(self.entry1.get()), int(self.entry2.get()), float(self.entry3.get()),
                         float(self.entry4.get()), float(self.entry5.get()))
    ...
    def showAbout(self):
        showinfo("About",
                 "An AI Tool for Student-Supervisor Allocation in Coventry University\n\nVersion: 1.0\nCreated By: Rithin Chalumuri \nSupervisor: Victor Sanchez \n\nThis application was created as part of Coventry University Summer Research Project 2017.")

    def showTutorial(self):
        showinfo("Tutorial",
                 "The find out how to use this application please go through 'tutorial.pdf' in the installation folder.")
